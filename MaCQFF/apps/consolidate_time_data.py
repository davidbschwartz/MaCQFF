...
workspace = "MaCQFF"
path = workspace + "/experiments/GP/environments"
...
for env in env_folders:
    ...
    for class_ in class_folders:
        ...
        for exp_num in experiments_folders:
            data_path = path_class + "/" + exp_num + "/time_dict.pkl"
...
a_file = open(path + "/0_time_data.pkl", "wb")
...
for env in env_folders:
    ...
    for class_ in class_folders:
        ...
        experiments_folders = [
            f.name for f in os.scandir(path_class) if f.is_dir()]
        for exp_num in experiments_folders:
            data_path = path_class + "/" + exp_num + "/cum_time_dict.pkl"
            if os.path.exists(data_path):
                if os.path.getsize(data_path) != 0:
                    k = open(data_path, "rb")
                    data = pickle.load(k)
...
a_file = open(path + "/0_cum_time_data.pkl", "wb")
pickle.dump(all_env_data, a_file)
a_file.close()